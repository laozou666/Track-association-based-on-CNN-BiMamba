...
MAP_IMAGE_PATH = os.path.join(DEMO_DIR, "地图3-毕设.png")

# 底图对应的经纬度范围（台湾以东海域）
MAP_GEO_EXTENT = [121.30, 123.90, 24.80, 26.10]
...
MAP_IMAGE = np.array(Image.open(MAP_IMAGE_PATH).convert("RGB"))
RESULT_CACHE = {}
...
def _tracks_overlap_map(view_bounds):
    lon_min, lon_max, lat_min, lat_max = view_bounds
    map_lon_min, map_lon_max, map_lat_min, map_lat_max = MAP_GEO_EXTENT
    return not (
        lon_max < map_lon_min
        or lon_min > map_lon_max
        or lat_max < map_lat_min
        or lat_min > map_lat_max
    )
...
def plot_result_on_map(df_a, df_b, prob, is_associated):
    view = _view_bounds(df_a, df_b)
    use_geo_extent = _tracks_overlap_map(view)
    img_extent = MAP_GEO_EXTENT if use_geo_extent else view
...
def predict_association(preset_name, file_a, file_b, progress=gr.Progress()):
    ...
    try:
        ...
        fig = plot_result_on_map(df_a, df_b, prob, is_associated)
...
底图使用 `地图2-毕设.png`；若航迹位于该图对应海域，将按真实经纬度叠加显示。
                """
            )
